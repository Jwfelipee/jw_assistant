---
name: midweek-assignment-system-task-01
description: Implementa exclusivamente a task 01 (Monorepo bootstrap) da change midweek-assignment-system. Use via /opsx-apply ou quando a task 01 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **01** da change `midweek-assignment-system`.
...
`openspec/changes/midweek-assignment-system/tasks/task-01-monorepo-bootstrap.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/midweek-assignment-system/tasks.md` as linhas desta task: `- [ ]` → `- [x]` (apenas subtarefas do grupo 01).
...
- `openspec/changes/midweek-assignment-system/tasks/task-01-monorepo-bootstrap.md` (apenas sua task)
