---
name: midweek-assignment-system-task-11
description: Implementa exclusivamente a task 11 (PWA shell and navigation) da change midweek-assignment-system. Use via /opsx-apply ou quando a task 11 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **11** da change `midweek-assignment-system`.
...
`openspec/changes/midweek-assignment-system/tasks/task-11-pwa-shell-and-navigation.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/midweek-assignment-system/tasks.md` as linhas desta task: `- [ ]` → `- [x]` (apenas subtarefas do grupo 11).
6. Responda com: resumo, checklist de critérios (✓/✗), saída da verificação, arquivos alterados.

## Frontend Design (obrigatório nesta task)

Antes de qualquer UI, leia e siga integralmente:
`c:/workspace/lab/jw_assistant/.agents/skills/frontend-design/SKILL.md`

Identidade visual do produto: assistente interno de congregação para programação da reunião do meio de semana (S-140). Mobile-first, pt-BR, sóbrio e confiável — NÃO use os defaults genéricos listados na skill (cream+terracotta, purple SaaS, etc.). Defina tokens (cor/tipo/layout) alinhados ao domínio antes de codar UI.
...
- `openspec/changes/midweek-assignment-system/tasks/task-11-pwa-shell-and-navigation.md` (apenas sua task)
