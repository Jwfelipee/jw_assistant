---
name: midweek-assignment-system-task-04
description: Implementa exclusivamente a task 04 (Congregation settings) da change midweek-assignment-system. Use via /opsx-apply ou quando a task 04 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **04** da change `midweek-assignment-system`.
...
`openspec/changes/midweek-assignment-system/tasks/task-04-congregation-settings.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/midweek-assignment-system/tasks.md` as linhas desta task: `- [ ]` → `- [x]` (apenas subtarefas do grupo 04).
...
- `openspec/changes/midweek-assignment-system/tasks/task-04-congregation-settings.md` (apenas sua task)
