---
name: midweek-assignment-system-task-05
description: Implementa exclusivamente a task 05 (Participants) da change midweek-assignment-system. Use via /opsx-apply ou quando a task 05 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **05** da change `midweek-assignment-system`.
...
`openspec/changes/midweek-assignment-system/tasks/task-05-participants.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/midweek-assignment-system/tasks.md` as linhas desta task: `- [ ]` → `- [x]` (apenas subtarefas do grupo 05).
...
- `openspec/changes/midweek-assignment-system/tasks/task-05-participants.md` (apenas sua task)
