---
name: midweek-assignment-system-task-07
description: Implementa exclusivamente a task 07 (Assignment catalog) da change midweek-assignment-system. Use via /opsx-apply ou quando a task 07 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **07** da change `midweek-assignment-system`.
...
`openspec/changes/midweek-assignment-system/tasks/task-07-assignment-catalog.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/midweek-assignment-system/tasks.md` as linhas desta task: `- [ ]` → `- [x]` (apenas subtarefas do grupo 07).
...
- `openspec/changes/midweek-assignment-system/tasks/task-07-assignment-catalog.md` (apenas sua task)
