---
name: midweek-assignment-system-task-02
description: Implementa exclusivamente a task 02 (Database schema and seed) da change midweek-assignment-system. Use via /opsx-apply ou quando a task 02 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **02** da change `midweek-assignment-system`.
...
`openspec/changes/midweek-assignment-system/tasks/task-02-database-schema-and-seed.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/midweek-assignment-system/tasks.md` as linhas desta task: `- [ ]` → `- [x]` (apenas subtarefas do grupo 02).
...
- `openspec/changes/midweek-assignment-system/tasks/task-02-database-schema-and-seed.md` (apenas sua task)
