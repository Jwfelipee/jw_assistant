---
name: midweek-assignment-system-task-10
description: Implementa exclusivamente a task 10 (S-140 PDF export) da change midweek-assignment-system. Use via /opsx-apply ou quando a task 10 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **10** da change `midweek-assignment-system`.
...
`openspec/changes/midweek-assignment-system/tasks/task-10-s140-pdf-export.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/midweek-assignment-system/tasks.md` as linhas desta task: `- [ ]` → `- [x]` (apenas subtarefas do grupo 10).
...
- `openspec/changes/midweek-assignment-system/tasks/task-10-s140-pdf-export.md` (apenas sua task)
