---
name: midweek-assignment-system-task-03
description: Implementa exclusivamente a task 03 (Auth single-user) da change midweek-assignment-system. Use via /opsx-apply ou quando a task 03 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **03** da change `midweek-assignment-system`.
...
`openspec/changes/midweek-assignment-system/tasks/task-03-auth-single-user.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/midweek-assignment-system/tasks.md` as linhas desta task: `- [ ]` → `- [x]` (apenas subtarefas do grupo 03).
...
- `openspec/changes/midweek-assignment-system/tasks/task-03-auth-single-user.md` (apenas sua task)
