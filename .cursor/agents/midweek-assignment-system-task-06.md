---
name: midweek-assignment-system-task-06
description: Implementa exclusivamente a task 06 (Absences) da change midweek-assignment-system. Use via /opsx-apply ou quando a task 06 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **06** da change `midweek-assignment-system`.
...
`openspec/changes/midweek-assignment-system/tasks/task-06-absences.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/midweek-assignment-system/tasks.md` as linhas desta task: `- [ ]` → `- [x]` (apenas subtarefas do grupo 06).
...
- `openspec/changes/midweek-assignment-system/tasks/task-06-absences.md` (apenas sua task)
