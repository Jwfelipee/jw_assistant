---
name: midweek-assignment-system-task-09
description: Implementa exclusivamente a task 09 (Assignment schedule UI) da change midweek-assignment-system. Use via /opsx-apply ou quando a task 09 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **09** da change `midweek-assignment-system`.
...
`openspec/changes/midweek-assignment-system/tasks/task-09-assignment-schedule-ui.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/midweek-assignment-system/tasks.md` as linhas desta task: `- [ ]` → `- [x]` (apenas subtarefas do grupo 09).
...
- `openspec/changes/midweek-assignment-system/tasks/task-09-assignment-schedule-ui.md` (apenas sua task)
